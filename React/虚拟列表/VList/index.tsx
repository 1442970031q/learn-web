...
export default class VirtualList extends React.Component<VirtualListProps> {
    ...
    height = this.props.height; // 页面展示的高度
    total = this.props.total; // 总条数
    estimatedRowHeight = this.props.estimateRowHeight; // 预设的卡片高度
    bufferSize = this.props.bufferSize || 5; // 缓冲的个数

    scrollingContainer = React.createRef<HTMLDivElement>(); // 滑动的ref

    // params that use for calculate visible content
    limit = Math.ceil(this.height / this.estimatedRowHeight); // 当前展示高度最大的展示条数
    originStartIdx = 0;
    startIndex = 0;
    endIndex = Math.min(
        this.originStartIdx + this.limit + this.bufferSize,
        this.total - 1
    ); //保证this.originStartIdx + this.limit + this.bufferSize没有超出最后的index

    // handle dynamic inner content height
    cachedPositions: CachedPosition[] = []; // 缓存的位置
    phantomContentRef = React.createRef<HTMLDivElement>(); // 站位div的ref
    actualContentRef = React.createRef<HTMLDivElement>(); // 实际的ref
    phantomHeight = this.estimatedRowHeight * this.total; // 站位div 的高度
    ...
    updateCachedPositions = () => {
        ...
        let cumulativeDiffHeight = this.cachedPositions[startIdx].dValue; // 找到
    ...
    getTransform = () =>
        `translate3d(0,${this.startIndex >= 1
